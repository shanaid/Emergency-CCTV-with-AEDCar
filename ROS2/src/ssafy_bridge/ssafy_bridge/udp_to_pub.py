...
from ssafy_bridge.qos import qos_sensor
...
class ssafy_bridge(Node):
    ...
    def __init__(self):
        ...
        #turtlebot status
        self.ego_status_publisher = self.create_publisher(TurtlebotStatus, 'turtlebot_status', qos_sensor)
        self.ego_status_udp=erp_udp_parser(self.ego_status_publisher,user_ip, 8202,'turtlebot_status')

        # imu
        self.imu_publisher = self.create_publisher(Imu, 'imu', qos_sensor)
